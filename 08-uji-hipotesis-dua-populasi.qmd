...
1. **Cakupan populasi yang kita definisikan pada penelitian kita**. Jika satu populasi pada penelitian kita adalah *â€œseluruh mahasiswa ITERAâ€*, maka dua populasi dapat menjadi *â€œseluruh mahasiswa ITERAâ€* dan *â€œseluruh mahasiswa UIN RILâ€*, dan sebagainya.
   
   Akan ada hal yang kita bandingkan dari kedua ataupun seluruh populasi tersebut. Dalam hal ini, kita akan membuktikan bahwa *â€œada atau tidaknya perbedaan karakteristik dari mahasiswa ITERA dan mahasiswa UIN RILâ€*. Perbedaan karakteristik ini dapat berupa parameter tertentu, seperti rata-rata, median, atau proporsi pada dua atau lebih populasi yang diujikan.
...
Sementara itu, hipotesis alternatif ($H_1$) menunjukkan kondisi **adanya perbedaan antara kedua populasi tersebut** â€” baik perbedaan yang memiliki arah (lebih besar atau lebih kecil), maupun yang tidak memiliki arah (tidak sama dengan). Perhatikan Tabel \@ref(tab:tab-hipotesis-alternatif-dua-populasi) berikutÖ«.
...
g. **Menarik Kesimpulan dan Memaknai Hasil**  
   ...
   **Interpretasi ke dalam bidang PWK:** meskipun proporsi pengguna sepeda motor di ITERA (â‰ˆ60,7%) tampak lebih tinggi dari UNILA (â‰ˆ58,4%), selisih tersebut tidak cukup signifikan secara statistik. Ini menunjukkan bahwa **budaya penggunaan kendaraan bermotor pribadi cenderung serupa** di kedua kampus. Dalam konteks perencanaan transportasi, hal ini mengindikasikan bahwa intervensi kebijakan untuk mendorong moda transportasi alternatif â€” seperti angkutan umum atau sepeda â€” kemungkinan perlu diformulasikan secara bersamaan untuk kedua kawasan kampus tersebut.
:::
