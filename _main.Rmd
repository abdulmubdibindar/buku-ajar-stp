...
```{r fig-menentukan-tingpeng, echo=FALSE, fig.cap='Diagram alir menentukan tingkat pengukuran variabel', fig.align='center'}
...
# Membaca data
data_uin <- read.csv2("datasets/DataUtama_mhsUINRIL.csv")
data_unila <- read.csv2("datasets/DataUtama_mhsUNILA.csv")

# Menghitung max dan min
max_uin <- max(data_uin$`biaya.dalam.sepekan`, na.rm = TRUE)
min_uin <- min(data_uin$`biaya.dalam.sepekan`, na.rm = TRUE)
max_unila <- max(data_unila$`biaya.dalam.sepekan`, na.rm = TRUE)
min_unila <- min(data_unila$`biaya.dalam.sepekan`, na.rm = TRUE)
...
# Membaca data
data_unila <- read.csv2("datasets/DataUtama_mhsUNILA.csv")
biaya_unila <- data_unila$biaya.dalam.sepekan
...
```{r fig-column-moda, fig.cap='Grafik batang tegak distribusi moda transportasi', fig.align='center', out.width='70%'}
barplot(tabel_moda,
...
```{r fig-bar-moda, fig.cap='Grafik batang mendatar distribusi moda transportasi', fig.align='center', out.width='70%'}
barplot(tabel_moda,
  main = "Distribusi Moda Transportasi Mahasiswa UINRIL",
  xlab = "Frekuensi",
...
```{r fig-stacked-bar, fig.cap='Grafik batang bertumpuk moda transportasi berdasarkan jenis kelamin', fig.align='center', out.width='75%'}
...
```{r fig-bar-fakultas, fig.cap='Grafik batang fakultas mahasiswa UINRIL', fig.align='center', out.width='85%'}
...
```{r fig-lollipop-fakultas, fig.cap='Grafik lollipop fakultas mahasiswa UINRIL', fig.align='center', out.width='85%'}
ggplot(tabel_fakultas, aes(x = reorder(Fakultas, -Frekuensi), y = Frekuensi)) +
...
```{r fig-treemap-moda, fig.cap='Treemap moda transportasi berdasarkan jenis kelamin', fig.align='center', out.width='80%'}
...
Untuk membuat grafik donat (*donut chart*), kita dapat menggunakan package `ggplot2`:
...
ggplot(df_moda, aes(x = 2, y = Frekuensi, fill = Moda)) +
  ...
  scale_fill_manual(values = c("#3498db", "#e74c3c", "#f39c12", "#2ecc71", "#9b59b6"))
```
...
# Konversi kolom biaya perjalanan (mengganti koma dengan titik)
data_itera$biaya.perjalanan <- as.numeric(gsub(",", ".", data_itera$biaya.perjalanan))

# Melihat statistik deskriptif
summary(data_itera$biaya.perjalanan)
```
...
hist(data_itera$biaya.perjalanan,
...
bins <- seq(0, max(data_itera$biaya.perjalanan, na.rm = TRUE), length.out = 10)

# Mengelompokkan data ke dalam bin
data_itera$bin <- cut(data_itera$biaya.perjalanan, breaks = bins, include.lowest = TRUE)
...
### Studi Kasus: Membuat Boxplot Uang Saku Mahasiswa {.unnumbered}

Boxplot menampilkan ukuran penyebaran data numerik secara grafis. Mari kita buat boxplot untuk menganalisis distribusi uang saku mahasiswa dari berbagai universitas.
...
# Menambahkan kolom identitas kampus
data_uinril$Kampus <- "UINRIL"
data_unila$Kampus <- "UNILA"

# Menggabungkan data
data_gabung <- rbind(data_uinril, data_unila)

# Melihat ringkasan data uang saku
summary(data_gabung$Uang.Saku)
```

Mari kita buat boxplot tunggal untuk uang saku UINRIL terlebih dahulu:

```{r fig-boxplot-single, fig.cap='Boxplot uang saku mahasiswa UINRIL', fig.align='center', out.width='60%'}
boxplot(data_uinril$Uang.Saku,
  main = "Distribusi Uang Saku Mahasiswa UINRIL",
  ylab = "Uang Saku (Rp juta)",
  col = "#3498db",
  border = "#2c3e50"
)

# Menambahkan label untuk setiap elemen boxplot
text(1, 1, quantile(data_uinril$Uang.Saku, 0, 25, na.rm = TRUE),
  labels = paste0("Q1 = ", round(quantile(data_uinril$Uang.Saku, 0, 25, na.rm = TRUE), 1)),
  pos = 4, cex = 0, 8, col = "red"
)
text(1, 1, median(data_uinril$Uang.Saku, na.rm = TRUE),
  labels = paste0("Median = ", round(median(data_uinril$Uang.Saku, na.rm = TRUE), 1)),
  pos = 4, cex = 0, 8, col = "red"
)
text(1, 1, quantile(data_uinril$Uang.Saku, 0, 75, na.rm = TRUE),
  labels = paste0("Q3 = ", round(quantile(data_uinril$Uang.Saku, 0, 75, na.rm = TRUE), 1)),
  pos = 4, cex = 0, 8, col = "red"
)
```

Sekarang mari kita buat boxplot ganda untuk membandingkan distribusi uang saku antar kampus:

```{r fig-boxplot-multiple, fig.cap='Boxplot uang saku mahasiswa berdasarkan kampus', fig.align='center', out.width='75%'}
boxplot(Uang.Saku ~ Kampus,
  data = data_gabung,
  main = "Perbandingan Distribusi Uang Saku Antar Kampus",
  xlab = "Kampus",
  ylab = "Uang Saku (Rp juta)",
  col = c("#3498db", "#e74c3c"),
  border = "#2c3e50"
...
**Interpretasi:** Dari boxplot ganda terlihat bahwa distribusi uang saku mahasiswa UNILA memiliki median yang sedikit lebih tinggi dibandingkan UINRIL. Rentang interkuartil (tinggi kotak) UNILA juga lebih besar, menunjukkan variasi uang saku yang lebih beragam. Terdapat beberapa outlier pada kedua kampus yang menunjukkan ada mahasiswa dengan uang saku yang jauh lebih tinggi dari mayoritas. Informasi ini berguna untuk perencanaan fasilitas kampus seperti kantin atau transportasi yang disesuaikan dengan daya beli mahasiswa.
...
### Studi Kasus: Membuat Scatterplot Hubungan Biaya Perjalanan dan Uang Saku {.unnumbered}

Scatterplot memperlihatkan hubungan antara dua variabel numerik. Setiap titik mewakili satu observasi dengan nilai pada sumbu X dan Y. Mari kita analisis hubungan antara biaya perjalanan dan uang saku mahasiswa ITERA.

```{r}
# Memuat data
data_itera <- read.csv2("datasets/DataUtama_mhsITERA.csv")

# Konversi biaya perjalanan (mengganti koma dengan titik)
data_itera$biaya.perjalanan <- as.numeric(gsub(",", ".", data_itera$biaya.perjalanan))

# Melihat struktur data untuk memahami pemetaan ke scatterplot
head(data_itera[, c("biaya.perjalanan", "Uang.Saku")], 10)
```

Tabel di atas menunjukkan pasangan nilai untuk setiap mahasiswa. Kolom pertama (`biaya.perjalanan`) akan menjadi sumbu X, dan kolom kedua (`Uang.Saku`) menjadi sumbu Y. Setiap baris akan menjadi satu titik pada scatterplot.

Mari kita buat scatterplot sederhana:

```{r fig-scatter-basic, fig.cap='Scatterplot hubungan biaya perjalanan dan uang saku', fig.align='center', out.width='75%'}
plot(data_itera$biaya.perjalanan, data_itera$Uang.Saku,
  main = "Hubungan Biaya Perjalanan dan Uang Saku Mahasiswa ITERA",
  xlab = "Biaya Perjalanan (Rp ribu)",
  ylab = "Uang Saku (Rp juta)",
  pch = 16,
...
abline(lm(Uang.Saku ~ biaya.perjalanan, data = data_itera),
...
- Nilai **kolom kedua** (uang saku) menentukan **posisi vertikal (Y)** titik
- Titik di koordinat (50, 2) artinya mahasiswa tersebut memiliki biaya perjalanan 50 ribu dan uang saku 2 juta

**Interpretasi:** Scatterplot menunjukkan adanya hubungan positif yang lemah hingga sedang antara biaya perjalanan dan uang saku mahasiswa. Mahasiswa dengan uang saku lebih besar cenderung memiliki biaya perjalanan yang lebih tinggi, yang masuk akal karena mereka mungkin tinggal lebih jauh dari kampus atau menggunakan moda transportasi yang lebih mahal. Garis tren (garis putus-putus merah) menunjukkan arah umum hubungan ini. Informasi ini berguna untuk perencanaan kebijakan kampus terkait subsidi transportasi yang mungkin lebih diprioritaskan untuk mahasiswa dengan uang saku rendah namun biaya perjalanan tinggi.
