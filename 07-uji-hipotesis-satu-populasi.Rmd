...
Melalui pengujian hipotesis, inferensi dilakukan dengan menarik kesimpulan untuk parameter berdasarkan hipotesis dari statistik kita. Secara sederhana, analisis diawali dengan pertanyaan: "jika nilai statistik ($\bar{x}$) adalah $a$, apakah nilai parameternya ($\mu$) juga $a$?" (Gambar \@ref(fig:fig-ilustrasi-hipotesis)). Bentuk pertanyaan ini bisa kita ubah juga menjadi bentuk pernyataan, atau hipotesis, yakni "nilai parameter kita adalah sama dengan statistik kita, yakni $a$ ($\mu = a$)." Kesimpulan yang kita akan ambil nanti hanya ada di antara dua pilihan: **menerima** atau **menolak** hipotesis tersebut.
...
Estimasi parameter dan uji hipotesis parameter **sama-sama bertujuan memperkirakan nilai parameter**. Akan tetapi, keduanya melakukannya  dengan cara yang berbeda. Estimasi parameter menghasilkan suatu **rentang nilai yang memuat parameter-parameter yang mungkin* bagi parameter populasi**. Pertanyaan yang dijawab biasanya berbentuk: "Berapa kira-kira nilai rata-rata populasi X?"

Sementara itu, uji hipotesis parameter berfokus pada penerimaan atau penolakan dugaan kita tentang hipotesis terhadap parameter. Pertanyaan yang diajukan lebih ke arah: "Jika saya menduga bahwa rata-rata populasi X adalah Y, apakah dugaan tersebut dapat diterima?"
...
Perlu diperhatikan bahwa kita akan menguji hipotesis untuk rata-rata. Oleh karena itu variabel kita harus bisa dipastikan bertingkat pengukuran minimal metrik. Variabel yang akan diuji di sini adalah skor kepuasan yang merupakan variabel interval/rasio, maka uji hipotesis parameter dapat dilanjutkan.
...
- Tingkat pengukuran variabel berada pada skala rasio, sehingga dilakukan uji hipotesis parameter rata-rata.
