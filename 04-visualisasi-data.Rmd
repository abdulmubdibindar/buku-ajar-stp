...
```{r bab-4-diagram-batang, echo=FALSE, message=FALSE, warning=FALSE}
# Memuat library
library(tidyverse)
library(knitr)
library(kableExtra)
...
# Memuat data
data_uinril <- read.csv("datasets/DataUtama_mhsUINRIL.csv", sep = ";")

# Membuat tabel distribusi frekuensi untuk moda transportasi
tabel_moda <- data_uinril |>
  count(kendaraan.utama, name = "Frekuensi")

# Format tabel
kbl(tabel_moda,
  col.names = c("Moda Transportasi", "Frekuensi"),
...
```{r fig-column-moda, echo=FALSE, fig.cap='Grafik batang tegak distribusi moda transportasi', fig.align='center', out.width='75%'}
ggplot(tabel_moda, aes(x = kendaraan.utama, y = Frekuensi)) +
  geom_col(fill = "#3498db") +
  theme_minimal() +
  labs(
    title = "Distribusi Moda Transportasi Mahasiswa UINRIL",
    x = "Moda Transportasi",
    y = "Frekuensi"
  ) +
  theme(
...
```{r fig-bar-moda, echo=FALSE, fig.cap='Grafik batang mendatar distribusi moda transportasi', fig.align='center', out.width='75%'}
ggplot(tabel_moda, aes(x = Frekuensi, y = kendaraan.utama)) +
...
```{r tab-distribusi-2-var, echo=FALSE, message=FALSE, warning=FALSE}
# Membuat tabel kontingensi (distribusi frekuensi dua variabel)
tabel_moda_jk <- data_uinril |>
  # Ubah jadi factor supaya urutan dan kolomnya PASTI, meskipun datanya 0
  mutate(kendaraan.utama = factor(kendaraan.utama, levels = c(
    "Berjalan kaki",
  ...
  # .drop = FALSE supaya yang jumlahnya 0 tetap dihitung
  count(jenis.kelamin, kendaraan.utama, .drop = FALSE, name = "Frekuensi") |>
  pivot_wider(
    names_from = kendaraan.utama,
    values_from = Frekuensi,
    values_fill = 0
  )
...
kbl(
  ...
  caption = "Distribusi Frekuensi Moda Transportasi Mahasiswa UIN Raden Intan Lampung berdasarkan Jenis Kelamin"
) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed"),
    latex_options = if (knitr::is_latex_output()) "HOLD_position" else c("striped", "hover"),
...
# Konversi data ke format long untuk ggplot2
tabel_moda_jk_long <- data_uinril |>
  count(jenis.kelamin, kendaraan.utama, name = "Frekuensi")

ggplot(tabel_moda_jk_long, aes(x = kendaraan.utama, y = Frekuensi, fill = jenis.kelamin)) +
  geom_col(position = "stack") +
  theme_minimal() +
  labs(
    title = "Moda Transportasi Berdasarkan Jenis Kelamin",
    x = "Moda Transportasi",
    y = "Frekuensi",
    fill = "Jenis Kelamin"
  ) +
...
Pada dataset dengan banyak kategori, grafik batang konvensional dapat menghasilkan efek MoirÃ© yang mengganggu. Mari kita bandingkan penggunaan grafik batang biasa dengan grafik lollipop menggunakan data program studi mahasiswa UINRIL.

```{r bab-4-distribusi-per-prodi, echo=FALSE, fig.align='center'}
...
data_uinril <- read.csv("datasets/DataUtama_mhsUINRIL.csv", sep = ";")
tabel_prodi <- data_uinril |>
  count(prodi, name = "Frekuensi") |>
  arrange(desc(Frekuensi))

# Format tabel (hanya 6 teratas)
kbl(
  head(tabel_prodi, 6),
  col.names = c("Nama Program Studi", "Frekuensi"),
  caption = "Distribusi Frekuensi Mahasiswa UIN Raden Intan Lampung Per Program Studi"
) |>
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed"),
    latex_options = if (knitr::is_latex_output()) "HOLD_position" else c("striped", "hover"),
  ...
  kableExtra::footnote(
    general = paste0("Menampilkan 6 dari ", nrow(tabel_prodi), " program studi"),
...
**Perbandingan:** Pada grafik lollipop, setiap kategori lebih mudah dibedakan karena hanya tiang tipis dan titik yang mewakili nilai, sehingga menghindari efek visual yang berlebihan (MoirÃ©). Grafik lollipop sangat cocok untuk dataset dengan banyak kategori seperti ini, karena lebih ringkas dan tetap informatif. Sementara grafik batang biasa terlihat lebih padat dan dapat menimbulkan kesulitan dalam membaca data ketika batang-batangnya berdekatan.
...
```{r tab-treemap-data, echo=FALSE, message=FALSE, warning=FALSE}
...
# Membuat tabel kontingensi menggunakan dplyr
tabel_moda_jk <- data_uinril |>
  count(jenis.kelamin, kendaraan.utama, name = "Frekuensi") |>
  rename(Jenis_Kelamin = jenis.kelamin, Moda_Transportasi = kendaraan.utama)

# Format tabel dengan kable
kbl(
  tabel_moda_jk,
  col.names = c("Jenis Kelamin", "Moda Transportasi", "Frekuensi"),
  caption = "Distribusi Frekuensi Moda Transportasi berdasarkan Jenis Kelamin untuk Treemap"
) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed"),
    latex_options = if (knitr::is_latex_output()) "HOLD_position" else c("striped", "hover"),
...
```{r tab-pie-data, echo=FALSE, message=FALSE, warning=FALSE}
# Memuat data
data_uinril <- read.csv("datasets/DataUtama_mhsUINRIL.csv", sep = ";")
...
kbl(
  tabel_moda,
  col.names = c("Moda Transportasi", "Frekuensi"),
  caption = "Distribusi Frekuensi Moda Transportasi untuk Grafik Pai"
) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed"),
    latex_options = if (knitr::is_latex_output()) "HOLD_position" else c("striped", "hover"),
...
```{r fig-pie-moda, echo=FALSE, fig.cap='Grafik pai moda transportasi mahasiswa UINRIL', fig.align='center', out.width='70%'}
# Menghitung persentase
tabel_moda_pct <- tabel_moda |>
  mutate(
    Persentase = round(100 * Frekuensi / sum(Frekuensi), 1),
    Label = paste0(kendaraan.utama, "\n", Persentase, "%")
  )

ggplot(tabel_moda_pct, aes(x = "", y = Frekuensi, fill = kendaraan.utama)) +
  geom_bar(stat = "identity", width = 1, color = "white") +
  coord_polar(theta = "y") +
  theme_void() +
  labs(title = "Distribusi Moda Transportasi Mahasiswa UINRIL") +
  geom_text(aes(label = Persentase),
    position = position_stack(vjust = 0.5),
    color = "white", size = 4
  ) +
  scale_fill_manual(
    values = c("#3498db", "#e74c3c", "#f39c12", "#2ecc71", "#9b59b6", "#34495e"),
    name = "Moda Transportasi"
  ) +
...
```{r fig-donut-moda, echo=FALSE, fig.cap='Grafik donat moda transportasi mahasiswa UINRIL', fig.align='center', out.width='70%'}
library(ggplot2)

# Menggunakan tabel yang sudah ada dan menghitung persentase
df_moda <- tabel_moda |>
  mutate(
    Persentase = round(100 * Frekuensi / sum(Frekuensi), 1),
...
# Membuat donut chart
ggplot(df_moda, aes(x = 2, y = Frekuensi, fill = kendaraan.utama)) +
  geom_bar(stat = "identity", width = 1, color = "white") +
  coord_polar(theta = "y") +
  xlim(0.5, 2.5) +
  theme_void() +
...
```{r bab-4-hist-biaya-prep, echo=FALSE, message=FALSE, warning=FALSE}
# Memuat data
data_itera <- read.csv("datasets/DataUtama_mhsITERA.csv", sep = ";")

# Rescale biaya ke dalam ribuan
data_itera$biaya.dalam.sepekan <- data_itera$biaya.dalam.ribu2

# Melihat statistik deskriptif
summary_stats <- data.frame(
  Statistik = c("Min", "Q1", "Median", "Mean", "Q3", "Max", "NA"),
  Nilai = c(
    min(data_itera$biaya.dalam.sepekan, na.rm = TRUE),
    quantile(data_itera$biaya.dalam.sepekan, 0.25, na.rm = TRUE),
    median(data_itera$biaya.dalam.sepekan, na.rm = TRUE),
    mean(data_itera$biaya.dalam.sepekan, na.rm = TRUE),
    quantile(data_itera$biaya.dalam.sepekan, 0.75, na.rm = TRUE),
    max(data_itera$biaya.dalam.sepekan, na.rm = TRUE),
    sum(is.na(data_itera$biaya.dalam.sepekan))
  )
)

kbl(
  summary_stats,
  col.names = c("Statistik", "Nilai (Rp ribu)"),
  caption = "Statistik Deskriptif Biaya Perjalanan Mahasiswa ITERA (dalam ribuan rupiah)",
  digits = 2
) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed"),
...
```{r fig-hist-biaya-auto, echo=FALSE, fig.cap='Histogram biaya perjalanan (bin otomatis) dalam ribuan rupiah', fig.align='center', out.width='75%'}
ggplot(data_itera, aes(x = biaya.dalam.sepekan)) +
  geom_histogram(fill = "#3498db", color = "white", bins = 30) +
  theme_minimal() +
  labs(
    title = "Distribusi Biaya Perjalanan Mahasiswa ITERA",
...
```{r fig-hist-biaya-manual, echo=FALSE, fig.cap='Histogram biaya perjalanan (bin manual) dalam ribuan rupiah', fig.align='center', out.width='75%'}
# Membuat histogram dengan 15 bin
ggplot(data_itera, aes(x = biaya.dalam.sepekan)) +
  geom_histogram(bins = 15, fill = "#e74c3c", color = "white") +
  theme_minimal() +
  labs(
    title = "Distribusi Biaya Perjalanan (15 Bin)",
    x = "Biaya Perjalanan (Rp ribu)",
    y = "Frekuensi"
  ) +
  theme(
...
```{r bab-4-tab-freq-manual, echo=FALSE, message=FALSE, warning=FALSE}
# Membuat interval bin menggunakan dplyr
data_itera_binned <- data_itera |>
  mutate(
    bin = cut(biaya.dalam.sepekan,
      breaks = seq(0, max(biaya.dalam.sepekan, na.rm = TRUE), length.out = 10),
      include.lowest = TRUE
    )
  )

# Membuat tabel frekuensi
tabel_freq <- data_itera_binned |>
  count(bin, name = "Frekuensi")

kbl(
  head(tabel_freq, 6),
  col.names = c("Interval Biaya (Rp ribu)", "Frekuensi"),
  caption = "Tabel Frekuensi Biaya Perjalanan dalam Ribuan Rupiah (6 Interval Pertama)"
) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed"),
    latex_options = if (knitr::is_latex_output()) "HOLD_position" else c("striped", "hover"),
...
```{r jenis-boxplot, echo=FALSE, out.width='40%', fig.align='center', fig.cap='*Boxplot* yang terdiri atas dua kategori objek: A dan B'}
knitr::include_graphics("figures/jenis-boxplot.png")
```
...
```{r bab-4-boxplot-prep, echo=FALSE, message=FALSE, warning=FALSE}
...
data_uinril <- data_uinril |>
  select(biaya.dalam.ribu2, kampus) |>
  rename(biaya.dalam.sepekan = biaya.dalam.ribu2)
...
summary_stats <- data.frame(
  Statistik = c("Min", "Q1", "Median", "Mean", "Q3", "Max", "NA"),
  Nilai = c(
    min(data_gabung$biaya.dalam.sepekan, na.rm = TRUE),
    quantile(data_gabung$biaya.dalam.sepekan, 0.25, na.rm = TRUE),
    median(data_gabung$biaya.dalam.sepekan, na.rm = TRUE),
    mean(data_gabung$biaya.dalam.sepekan, na.rm = TRUE),
    quantile(data_gabung$biaya.dalam.sepekan, 0.75, na.rm = TRUE),
    max(data_gabung$biaya.dalam.sepekan, na.rm = TRUE),
    sum(is.na(data_gabung$biaya.dalam.sepekan))
  )
)
...
kbl(
  ...
  caption = "Statistik Deskriptif Biaya Perjalanan Gabungan (UINRIL & UNILA)",
  digits = 2
) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed"),
...
```{r fig-boxplot-single, echo=FALSE, message=FALSE, warning=FALSE, fig.cap='Boxplot biaya perjalanan mahasiswa UINRIL', fig.align='center', out.width='60%'}
ggplot(data_uinril, aes(y = biaya.dalam.sepekan)) +
  geom_boxplot(fill = "#3498db", color = "#2c3e50", width = 0.5) +
  theme_minimal() +
  labs(
    title = "Distribusi Biaya Perjalanan Mahasiswa UINRIL",
    y = "Biaya Perjalanan (Rp ribu)"
  ) +
  theme(
    plot.title = element_text(face = "bold", hjust = 0.5),
...
```{r fig-boxplot-multiple, echo=FALSE, message=FALSE, warning=FALSE, fig.cap='Boxplot biaya perjalanan mahasiswa berdasarkan kampus', fig.align='center', out.width='75%'}
ggplot(data_gabung, aes(x = kampus, y = biaya.dalam.sepekan, fill = kampus)) +
  geom_boxplot(color = "#2c3e50") +
  theme_minimal() +
  labs(
    title = "Perbandingan Distribusi Biaya Perjalanan Antar Kampus",
    x = "Kampus",
    y = "Biaya Perjalanan (Rp ribu)"
  ) +
...
- ***Whiskers* (garis atas dan bawah)**: Menunjukkan rentang data yang masih dalam batas normal (1,5 Ã— IQR)
...
```{r line-plot-area-plot, echo=FALSE, fig.align='center', fig.cap='Ilustrasi line plot dan area plot serta data terstruktur yang membentuknya'}
knitr::include_graphics("figures/diagram-garis-area.png")
```
...
```{r fig-area-plot-multi, echo=FALSE, out.width='60%', fig.align='center', fig.cap='Ilustrasi area plot dengan tiga variabel'}
knitr::include_graphics("figures/area-plot-lebih-dari-1.png")
```
...
```{r bab-4-scatter-prep, echo=FALSE, message=FALSE, warning=FALSE}
# Memuat library
library(tidyverse)
...
# Gunakan data biaya dalam ribu dan jarak km langsung
data_itera$biaya.dalam.sepekan <- data_itera$biaya.dalam.ribu2
data_itera$jarak <- data_itera$jarak.km
...
kbl(
  ...
  col.names = c("Biaya Perjalanan (Rp ribu)", "Jarak Tempuh (km)"),
  caption = "Contoh Data Biaya Perjalanan dan Jarak Tempuh dalam Ribuan Rupiah (10 Observasi Pertama)",
  digits = 2
) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed"),
...
```{r fig-scatter-ggplot, echo=FALSE, message=FALSE, fig.cap='Scatterplot jarak tempuh vs biaya perjalanan dalam ribuan rupiah', fig.align='center', out.width='75%'}
library(ggplot2)
...
```{r simpulan-memilih-visualisasi, echo=FALSE, fig.cap='Panduan memilih jenis visualisasi', fig.align='center'}
knitr::include_graphics("images/CrazyEgg-Diagram.webp")
```
