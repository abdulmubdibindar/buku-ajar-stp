...
```{r 05-inferensia-1-sampling-studi-kasus-setup-data, echo=FALSE, message=FALSE, warning=FALSE}
library(readr)
library(dplyr)
library(knitr)
...
data_populasi <- data_itera |>
  mutate(
    ...
    Tahun_Kuliah = case_when(
      tingkat.semester == "Semester 1 - 2" ~ "Pertama",
      tingkat.semester == "Semester 3 - 4" ~ "Kedua",
      tingkat.semester == "Semester 5 - 6" ~ "Ketiga",
      tingkat.semester == "Semester 7 - 8 ke atas" ~ "Keempat"
    )
  ) |>
  filter(!is.na(jarak))
...
```{r 05-inferensia-1-sampling-studi-kasus-srs-demo, echo=FALSE}
set.seed(123)
...
```{r 05-inferensia-1-sampling-studi-kasus-sys-demo, echo=FALSE}
...
```{r 05-inferensia-1-sampling-studi-kasus-strat-demo, echo=FALSE}
...
```{r 05-inferensia-2-clt-studi-kasus-simulasi, echo=FALSE, fig.cap="Perbandingan Distribusi Populasi (Kiri) dan Distribusi Sampling (Kanan)", fig.width=10}
library(ggplot2)
library(gridExtra)
