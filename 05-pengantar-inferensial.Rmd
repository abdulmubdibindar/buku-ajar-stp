...
# 2. Binomial
p2 <- ggplot(data.frame(x = 0:10, y = dbinom(0:10, size = 10, prob = 0.5)), aes(x, y)) +
  geom_col(fill = "#56B4E9", alpha = 0.8, width = 0.7) +
  labs(title = "Binomial") +
  theme_dist
...
p5 <- ggplot(data.frame(x = c(0, 5)), aes(x)) +
  stat_function(fun = df, args = list(df1 = 5, df2 = 20), 
                geom = "area", fill = "#56B4E9", alpha = 0.8) +
...
grid.arrange(p1, p2, p3, p4, p5, p6, ncol = 3,
             left = textGrob("Frekuensi", rot = 90, gp = gpar(fontsize = 12, fontface = "bold")),
             bottom = textGrob("Nilai Variabel", gp = gpar(fontsize = 12, fontface = "bold")))
```
...
# Gunakan range yang cukup lebar untuk menampung distribusi yang lebih lebar (sigma2)
...
create_normal_plot <- function(mu, sigma, fill_color, title_text) {
  ggplot(data.frame(x = x_range), aes(x)) +
    ...
    # Garis Mean
    geom_vline(xintercept = mu, linetype = "dashed", color = "indianred", size = 0.5) +
    annotate("text", x = mu, y = dnorm(mu, mean=mu, sd=sigma) * 0.1, 
             label = expression(mu), color = "indianred", size = 4, fontface = "bold") +
    
    # Garis +/- 1 SD
    geom_vline(xintercept = c(mu - sigma, mu + sigma), linetype = "dashed", color = "indianred", size = 0.5) +
    annotate("text", x = mu - sigma, y = dnorm(mu - sigma, mean=mu, sd=sigma) * 0.2, 
             label = expression(mu - sigma), color = "indianred", size = 3, hjust = 1.1) +
    annotate("text", x = mu + sigma, y = dnorm(mu + sigma, mean=mu, sd=sigma) * 0.2, 
             label = expression(mu + sigma), color = "indianred", size = 3, hjust = -0.1) +

    # Garis +/- 2 SD
    geom_vline(xintercept = c(mu - 2*sigma, mu + 2*sigma), linetype = "dashed", color = "indianred", size = 0.5) +
    annotate("text", x = mu - 2*sigma, y = dnorm(mu - 2*sigma, mean=mu, sd=sigma) * 0.5, 
             label = expression(mu - 2*sigma), color = "indianred", size = 3, hjust = 1.1) +
    annotate("text", x = mu + 2*sigma, y = dnorm(mu + 2*sigma, mean=mu, sd=sigma) * 0.5, 
             label = expression(mu + 2*sigma), color = "indianred", size = 3, hjust = -0.1) +

    # Garis +/- 3 SD
    geom_vline(xintercept = c(mu - 3*sigma, mu + 3*sigma), linetype = "dashed", color = "indianred", size = 0.5) +
    annotate("text", x = mu - 3*sigma, y = dnorm(mu - 3*sigma, mean=mu, sd=sigma) * 0.8, 
             label = expression(mu - 3*sigma), color = "indianred", size = 3, hjust = 1.1) +
    annotate("text", x = mu + 3*sigma, y = dnorm(mu + 3*sigma, mean=mu, sd=sigma) * 0.8, 
             label = expression(mu + 3*sigma), color = "indianred", size = 3, hjust = -0.1)
}

# Plot 1: Sigma Kecil
p4 <- create_normal_plot(mu, sigma1, "#56B4E9", paste("Distribusi Normal (SD =", sigma1, ")"))

# Plot 2: Sigma Besar
p5 <- create_normal_plot(mu, sigma2, "#E69F00", paste("Distribusi Normal (SD =", sigma2, ")")) +
...
df_kurt <- data.frame(
  ...
  Type = rep(c("Normal (Kurtosis = 3)", "Platikurtik (Kurtosis = 0)"), each=200)
)
...
ggplot(df, aes(x=x, y=y)) +
  ...
  # 95% Line (Middle)
  annotate("segment", x = mu - 2*sigma, xend = mu + 2*sigma, y = 0.05, yend = 0.05, 
  ...
  annotate("text", x = mu, y = 0.05, label = "95%", vjust = -0.8, color="black", fontface="bold", size=4) +
   ...
   # 99.7% Line (Bottom)
  annotate("segment", x = mu - 3*sigma, xend = mu + 3*sigma, y = 0.01, yend = 0.01, 
           arrow = arrow(ends = "both", length = unit(0.2, "cm")), color="black", size=0.8) +
...
ggplot(sampel_satu, aes(x=Jarak)) +
  ...
  labs(title="Distribusi Objek (Sampel Tunggal n=10)",
...
ggplot(data.frame(x=hasil_simulasi), aes(x=x)) +
  ...
  labs(title="Distribusi Statistik Hasil Sampel",
...
ggplot(df_compare, aes(x = means, fill = type)) +
  ...
  labs(title = "Perbandingan Lebar Distribusi Statistik (n=30 vs n=100)",
...
Dari sampel ini, kita peroleh rata-rata jarak $\bar{x} = `r round(mean_sampel, 2)`$ km. Kita tahu simpangan baku populasi $\sigma = `r round(sigma, 2)`$. Maka **Standard Error** teoritisnya adalah:
...
```{r 05-inferensia-4-zscore-studi-kasus-hitung, echo=FALSE}
se_teoritis <- sigma / sqrt(n_sampel)
z_score <- (mean_sampel - mu) / se_teoritis
probabilitas <- pnorm(z_score)
```

Hasil: - Z-score = **`r round(z_score, 2)`** - Probabilitas ($P(Z < z)$) = **`r round(probabilitas, 4)`**
