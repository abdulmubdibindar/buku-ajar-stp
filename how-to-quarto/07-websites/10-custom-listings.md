...
source: "https://quarto.org/docs/websites/website-listings-custom.html"
author:
published:
created: 2026-06-09
description:
...
Quarto passes the listing contents to the template as an array called `items`. Each item is populated with the fields described in [Listing Fields](https://quarto.org/docs/websites/website-listings.html#listing-fields), plus any custom metadata from the document.

The template uses [EJS](https://ejs.co/) to combine HTML and JavaScript (inside `<% %>` tags) to process `items`. For example, this template constructs a list of links by looping through `items` and accessing the `path` and `title` fields:
...
![](https://quarto.org/docs/websites/images/listing-custom-metadata.png)
...
Portions of this website are built using custom listings. The best place to start is with [our gallery](https://quarto.org/docs/gallery/), which is a listing built using a custom template and a metadata file. You can view the source code used to create the gallery page in our [Github repository](https://github.com/quarto-dev/quarto-web).
