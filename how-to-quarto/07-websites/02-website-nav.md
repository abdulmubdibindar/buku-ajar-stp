...
source: "https://quarto.org/docs/websites/website-navigation.html"
author:
published:
created: 2026-06-09
description:
...
![A navigation bar. The title 'My Site' is on the left. To the right of the title are the words 'Home', 'Talks', and 'About'. 'Home' is slightly lighter than the other two words. On the far right side of the navigation bar is a search box.](https://quarto.org/docs/websites/images/nav-bar-dark.png)
...
| --- | --- |
| `logo` | Logo image to be displayed left of the title. When using a [`_brand.yml`](https://quarto.org/docs/authoring/brand.html) with a logo, the brand logo is automatically displayed. Use `logo: false` to suppress the brand logo on the navbar. |
...
| `tools` | List of navbar tools (e.g., link to github or bluesky, etc.). See [Navbar Tools](https://quarto.org/docs/websites/website-navigation.html#navbar-tools) for details. |
...
For more information on controlling the appearance of the navigation bar using HTML themes, see [HTML Themes - Navigation](https://quarto.org/docs/output-formats/html-themes.html#navigation).
...
![The right section of a Quarto navbar containing a Bluesky and Github logo. The Github logo is selected and a menu is underneath it with two items: 'Source Code' and 'Report a Bug'](https://quarto.org/docs/websites/images/navbar-tools-dark.png)
...
|  | ![A screenshot of a Quarto document where the sidebar has a white background and is closer to the body text.](https://quarto.org/docs/websites/images/nav-side-floating-dark.png) |
| --- | --- |
...
| `logo` | Optional logo image. When using a [`_brand.yml`](https://quarto.org/docs/authoring/brand.html) with a logo, the brand logo is automatically displayed. Use `logo: false` to suppress the brand logo on the sidebar. |
| `logo-alt` | Alternate text for the logo image. |
...
For more information on controlling the appearance of the side navigation using HTML themes, see [HTML Themes - Navigation](https://quarto.org/docs/output-formats/html-themes.html#navigation). If you need to control the width of the sidebar, see [Page Layout - Grid Customization](https://quarto.org/docs/output-formats/page-layout.html#grid-customization).
...
![A Quarto sidebar showing the site title with Bluesky and GitHub icons. The GitHub icon is selected and a dropdown menu shows two items: 'Source Code' and 'Report a Bug'. Below are a search box and navigation sections.](https://quarto.org/docs/websites/images/tools-dark.png)
...
![A navigation bar titled 'ProjectX' on the left. To the right of the title are the menu items 'Home', 'Tutorials', 'How-To', 'Fundamentals', and 'Reference'. 'Home' is in a lighter color than the other menu options. The other menu options have a triangle pointing down next to each one, indicating the existence of a drop-down menu. There is a search bar on the far right side of the navigation bar.](https://quarto.org/docs/websites/images/nav-bar-hybrid-dropdown-dark.png)
...
![Screenshot of a webpage. Above the header 'Tutorials' is the linked text 'Tutorials > Tutorial Landing'.](https://quarto.org/docs/websites/images/nav-breadcrumbs-dark.png)
...
Or to control page navigation for all pages in a directory specify `page-navigation` in [`_metadata.yml`](https://quarto.org/docs/projects/quarto-projects.html#directory-metadata):
...
![Reader mode toggle appearing in the top navigation with the table of contents collapsed into an 'On this page' dropdown.](https://quarto.org/docs/websites/images/reader-mode-dark.png)
...
![A screen shot of a Quarto document. Underneath the page table of contents on the right side are three options: 'Edit this page', 'View source' and 'Report an issue'. There is a Github icon to the left of 'Edit this Page.'](https://quarto.org/docs/websites/images/repo-actions-dark.png)
...
Here are some examples of how various popular websites handle custom 404 pages: [https://blog.fluidui.com/top-404-error-page-examples/](https://blog.fluidui.com/top-404-error-page-examples/).
...
Note that if you are already providing a `site-url` (which is required for generation of sitemaps and [social metadata](https://quarto.org/docs/websites/website-tools.html#social-metadata) preview images) then it’s enough to simply include the path within the `site-url`:
