---
title: "Website Tools"
source: "https://quarto.org/docs/websites/website-tools.html"
author:
published:
created: 2026-06-09
description:
...
![Screenshot showing an announcement bar on Quarto website with an 'info-circle' icon, a dismiss button, and a primary colour type. The content reads 'Alert - this is some information that you should pay attention to'.](https://quarto.org/docs/websites/images/website-announcement.png)
...
In this case, Quarto will automatically generate a title, description, and preview image for the content. For more information about how Quarto finds preview images, see [Preview Images](https://quarto.org/docs/websites/website-tools.html#preview-images).
...
| --- | --- |
| `storage` | **cookies -** Use cookies to store unique user and session identification (default).  **none -** Do not use cookies to store unique user and session identification. |
...
Google Analytics uses cookies to distinguish unique users and sessions. If you choose to use cookies to store this user data, you should consider whether you need to enable [Cookie Consent](https://quarto.org/docs/websites/website-tools.html#cookie-consent) in order to permit the viewer to control any tracking that you enable.
...
The user’s cookie preferences will automatically control [Google Analytics](https://quarto.org/docs/websites/website-tools.html#google-analytics) and [Algolia search analytics](https://quarto.org/docs/websites/website-search.html#algolia-insights) (if enabled) and can be used to control custom scripts you add as well (see [Custom Scripts and Cookie Consent](https://quarto.org/docs/websites/website-tools.html#custom-scripts-and-cookie-consent)). You can enable the default request for cookie consent using the following:
...
| `type` | The type of consent that should be requested, using one of these two values:  **express -** (default) This will block cookies until the user expressly agrees to allow them (or continue blocking them if the user doesn’t agree).  **implied -** This will notify the user that the site uses cookies and permit them to change preferences, but not block cookies unless the user changes their preferences. |
| --- | --- |
...
When cookie consent is enabled, Google Analytics and Algolia search analytics will use cookies based on user preference: for `type: express`, the default, they will not use cookies unless a user opts-in; for `type: implied`, they will use cookies unless a user opts-out. Without cookie consent enabled, Algolia will use session-only tracking without cookies. See [Algolia Insights](https://quarto.org/docs/websites/website-search.html#algolia-insights) for more details.
...
2. Add a `cookie-consent` attribute to your script tag, setting it one of the following 4 levels:
	...
	| `tracking` | Scripts that are used to track users, for example [Google Analytics](https://quarto.org/docs/websites/website-tools.html#google-analytics). |
...
- `404.html`, one option for providing a [404 Page](https://quarto.org/docs/websites/website-navigation.html#pages-404)
...
- `CNAME`, a file used by some publishing providers to specify a custom domain, e.g. [GitHub Pages](https://quarto.org/docs/publishing/github-pages.html#custom-domain)
- `.nojekyll`, a file used by GitHub pages to bypass building with Jekyll, e.g. when [publishing from `docs/`](https://quarto.org/docs/publishing/github-pages.html#render-to-docs)
...
| ![A Quarto sidebar showing a light theme. The 'Dark mode' toggle is off.](https://quarto.org/docs/websites/images/site-light.png) | ![A Quarto sidebar showing a dark theme. The 'Dark mode' toggle is on.](https://quarto.org/docs/websites/images/site-dark.png) |
| --- | --- |
