...
![Screenshot of the HTML preview from the document with the above metadata.](https://quarto.org/docs/authoring/images/scholarly-front-matter-html.png)

Not all of the metadata keys are used in every format. However, the tags described on this page will generally be supported in [journal article formats](https://quarto.org/docs/extensions/listing-journals.html). Currently the JATS format makes use of the broadest set of metadata tags, so if you want to check how things render we recommend previewing with `format: jats`.
...
You can read about the other keys you can provide to `author` and `affiliation` in the corresponding [Author](https://quarto.org/docs/authoring/front-matter.html#author) and [Affiliation](https://quarto.org/docs/authoring/front-matter.html#affiliation) sections below.
...
Notice that each element of `author` and `affiliation` is prefaced by a `-` and indented appropriately. You can read more about shortcuts to avoid repetition when authors share affiliations in the [Multiple Authors](https://quarto.org/docs/authoring/front-matter.html#multiple-authors) section below.

NoteSingular or Plural?
...
| `roles` | string(s) | Author’s roles. Read more in [Author Roles](https://quarto.org/docs/authoring/front-matter.html#roles) below. |
...
| `id` | string | An identifier to be used to refer to this author in other fields. See an example in [Funding](https://quarto.org/docs/authoring/front-matter.html#funding). |
...
Or alternatively, make use of the [Contributor Roles Taxonomy (CRediT)](https://credit.niso.org/). To use CRediT roles provide one of the [14 contributor](https://quarto.org/docs/authoring/front-matter.html#credit-roles) roles, e.g.:
...
TipExpand to see the 14 Contributor Roles
...
| `isni`   `ringgold`   `ror` | Numeric   Numeric   String | Affiliation IDs:   16 digit [International Standard Name Identifier (ISNI)](https://isni.org/);   4-6 digit [Ringgold ID](https://www.ringgold.com/ringgold-identifier/);   [Research Organization Registry (ROR) ID](https://ror.org/), starting with `https://ror.org/`, followed by a 9 digit alphanumeric identifier. |
...
The available abbreviations are covered in the [Creative Commons](https://quarto.org/docs/authoring/front-matter.html#cc) section below.
