...
| [Quarto Visual Editor](https://quarto.org/docs/visual-editor/content.html#editing-tables) | Visual editor for `.qmd` files with table editing support. |
...
List tables provide an alternative syntax for creating tables with complex content like multiple paragraphs, code blocks, or lists. Unlike [grid tables](https://quarto.org/docs/authoring/tables.html#grid-tables), list tables use familiar bullet list syntax, making them easier to write and maintain.
...
| `sum()` | Add values:  ```python sum([1, 2, 3]) ``` |
| --- | --- |
...
See [Table 3](https://quarto.org/docs/authoring/tables.html#tbl-fruits).
...
<table><tbody><tr><td rowspan="2">Citrus</td><td>Orange</td><td>0.90</td></tr><tr><td>Lemon</td><td>0.80</td></tr><tr><td rowspan="2">Stone Fruit</td><td>Peach</td><td>1.20</td></tr><tr><td>Plum</td><td>1.00</td></tr></tbody></table>
...
![Screenshot of PDF output showing a table with a caption and two columns. The column headers are Header 1 and Header 2, and the cell contents are an image of an elephant and text that reads Regular Output.](https://quarto.org/docs/authoring/images/raw-table-pdf.png)
...
For HTML raw blocks, this is done by providing a data attribute `qmd` or `qmd-base64` in an embedded `span` or `div` node. These nodes can appear anywhere that such content is allowed: table headers, footers, cells, captions, *etc.*
...
| ![](https://www.youtube.com/watch?v=wo9vZccmqwc) | Regular output |
| --- | --- |

#### LaTeX Raw Blocks

For LaTeX raw blocks (used in PDF output), the equivalent syntax is `\QuartoMarkdownBase64{<<base64-encoded-content>>}`. When Quarto detects this command, it decodes the base64 content, processes it as Quarto markdown (including cross-references, shortcodes, and user filters), and inserts the result back into the LaTeX output.
...
<table><tbody><tr><td colspan="2">I span two columns</td><td>C1</td></tr><tr><td rowspan="2">I span two rows</td><td>B2</td><td>C2</td></tr><tr><td>B3</td><td>C3</td></tr></tbody></table>
