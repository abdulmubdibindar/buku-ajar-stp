...
We’ll start by covering the basics of Quarto, then delve into the differences between Quarto and R Markdown in the sections on [Chunk Options](https://quarto.org/docs/computations/r.html#chunk-options) and [Output Formats](https://quarto.org/docs/computations/r.html#output-formats) below.
...
1. Using the **Render** button in RStudio:
	![The top section of a qmd file as displayed in RStudio. There is a toolbar right above the document containing various options, including 'Render.' There is a stylized, segmented blue arrow pointing at the word.](https://quarto.org/docs/tools/images/rstudio-render.png)
...
You can download RStudio from [https://posit.co/download/rstudio-desktop/](https://posit.co/download/rstudio-desktop/).

### Creating Documents

Use the **File:** **New File: Quarto Document…** command to create new Quarto documents:

![The 'New Quarto Document' dialog menu in RStudio.](https://quarto.org/docs/tools/images/new-quarto-doc.png)
...
![Screen shot of qmd file open in Positron with source markdown shown in Editor pane and a rendered document shown in the Viewer Pane.](https://quarto.org/docs/get-started/hello/images/positron-hello-r.png-dark.png)

The extension includes a **Quarto: Preview** command that can be accessed via the Command Palette, the keyboard shortcut , or a **Preview** button (![Preview icon](https://quarto.org/docs/tools/images/vscode-preview-icon.svg)) in the editor toolbar. After rendering, a preview is displayed in the Viewer pane within Positron.
...
![Screen shot of qmd file open in VS code with source markdown shown in left pane and output plot shown in the right.](https://quarto.org/docs/computations/images/r-vscode.png)
...
The VS Code extension includes a **Quarto: Preview** command that can be accessed via the Command Palette, the keyboard shortcut , or a **Preview** button (![Preview icon](https://quarto.org/docs/tools/images/vscode-preview-icon.svg)) in the editor. After rendering, a preview is displayed in a pane within VS Code alongside your document.

You can read more about using VS Code in [Tools: VS Code](https://quarto.org/docs/tools/vscode/).
