...
Adding parameter definitions to a document works differently depending on whether you are using the [Jupyter](https://jupyter.org/), [Knitr](https://yihui.name/knitr) or Julia engine.
...
Each key is evaluated as a constant and can be accessed under that name. Note that this means only keys that are valid Julia variable names are allowed. For example, `some_key` is valid while `some-key` isn’t, even though the latter is a valid YAML key.
