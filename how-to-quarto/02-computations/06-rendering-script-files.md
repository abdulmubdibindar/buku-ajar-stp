...
source: "https://quarto.org/docs/computations/render-scripts.html"
author:
published:
created: 2026-06-09
description:
...
On this page you can learn the [syntax](https://quarto.org/docs/computations/render-scripts.html#syntax) for Jupyter and Knitr notebook scripts, how to [Render and Preview](https://quarto.org/docs/computations/render-scripts.html#render-and-preview) notebook scripts as well as using notebook [Scripts in Projects](https://quarto.org/docs/computations/render-scripts.html#scripts-in-projects).
...
Scripts rendered with Quarto must begin with a markdown cell containing the YAML header.
...
A markdown cell containing a multiline string (`"""`).

4

Code cell options specified with `#|` comments.

5

You can include blank lines within cells—cells continue until another cell is encountered.
...
Scripts rendered with Quarto must begin with a YAML header using `#'` comments.

2

R code is the main content of the R script and is included without any delimitation.

3

Preface Markdown content with the `#'` comment.

4

Code cell options are specified with `#|` comments and apply to code below them.
...
The script must begin with the appropriate [syntax](https://quarto.org/docs/computations/render-scripts.html#syntax) for the YAML block to be rendered with Quarto. Based on the syntax detected Quarto will render with the Jupyter or Knitr engine.
...
Notebook scripts can also be included within [projects](https://quarto.org/docs/projects/quarto-projects.html) (e.g. websites, blogs, etc.). Scripts within projects are only rendered by Quarto when they start with the appropriate [syntax](https://quarto.org/docs/computations/render-scripts.html#syntax).
