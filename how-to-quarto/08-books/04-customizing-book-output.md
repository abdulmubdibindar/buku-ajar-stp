...
source: "https://quarto.org/docs/books/book-output.html"
author:
published:
created: 2026-06-09
description:
...
- [Site Analytics](https://quarto.org/docs/websites/website-tools.html#analytics)
...
- The [Hypothesis](https://web.hypothes.is/) commenting bar appears on the far right of the page. Note that [commenting](https://quarto.org/docs/output-formats/html-basics.html#commenting) is a feature available for all Quarto HTML output so appears in its own YAML key.

#### Sidebar Options

Note that books utilize the standard sidebar component from [Quarto Websites](https://quarto.org/docs/websites/). This means that you can use any of the available [sidebar options](https://quarto.org/docs/websites/website-navigation.html#side-navigation) within your `book` configuration. For example, here we specify a docked sidebar with a light background:
...
You can disable the brand logo on the sidebar if you’re using a [`_brand.yml`](https://quarto.org/docs/authoring/brand.html):
...
Quarto books can be rendered to PDF using [Typst](https://typst.app/). When you specify `format: typst` for a book project, Quarto automatically uses the bundled [orange-book](https://typst.app/universe/package/orange-book) format extension, which provides textbook styling.
...
The orange-book format provides:

- Chapter and section styling with decorative headers
- Support for book parts and appendices
- Chapter-based numbering for figures, tables, equations, and cross-references
- A styled table of contents
- Integration with [`_brand.yml`](https://quarto.org/docs/authoring/brand.html) for color and logo

### Customizing Typst Books

You can create your own Typst book format extension. See [Custom Typst Formats](https://quarto.org/docs/output-formats/typst-custom.html) for details on creating format extensions.
...
You can switch to KOMA `scrbook` as demonstrated above, or to the standard LaTeX `book` and `report` classes. You can find a summary of the differences between `book` and `report` here: [https://tex.stackexchange.com/questions/36988](https://tex.stackexchange.com/questions/36988)
